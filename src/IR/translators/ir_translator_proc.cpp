...
#include "IR/translators/ir_translator_proc.hpp"
...
void IR_Translator_Process::translate_process_vector_to_ir_instructions(struct process * proc){
    ...
    std::string header_name_proc = std::string(proc->name_process);
    

    // -- 1. Realizar bucle para cada proceso
    for(int i=start_index; i<=end_index; i++){
        // 8. Emitir instruccion de fin de proceso
        IR_operand op_id = instructions.emit_operand_id_thread(i-1);
        instructions.emit_instruction(IR_START_PROCESS,op_id);

        // -----------------------------------------------------------------------

        // 1. Obtener nombre de proceso
        std::string id_label_block = instructions.get_next_label_id();
        std::string proc_name = header_name_proc + "_" + std::to_string(i);
        ...
        // 8. Emitir instruccion de fin de proceso
        instructions.emit_instruction(IR_END_PROCESS,op_id);
...
void IR_Translator_Process::translate_list_process_to_ir_instructions(struct process * list_proc){
    ...
    while(current_proc != NULL){
        ...
        if(current_proc->index_identifier){
            ...
            // 8. Emitir instruccion de fin de proceso
            IR_operand op_id = instructions.emit_operand_id_thread(id_thread_manager.get_id());
            instructions.emit_instruction(IR_START_PROCESS,op_id);
