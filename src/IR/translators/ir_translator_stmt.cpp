...
#include "IR/translators/ir_translator_stmt.hpp"
...
void IR_Translator_Statement::translate_statement_to_ir_instructions(struct statement * stmt, bool from_subprogram){
   // -- Comprobar tipo de sentencias
    ...
    {   
        ...
        instructions.emit_instruction(IR_WAIT_PROCESS);
    ...
    case STMT_SLEEP:
    {
        this->translate_statement_sleep_to_ir_instructions(stmt,from_subprogram);
        break;
    }
    case STMT_SEM_WAIT:
    {
...
void IR_Translator_Statement::translate_statement_for_to_ir_instructions(struct statement * stmt, bool from_subprogram){
    ...
    instructions.emit_instruction(IR_OP_ADD_I,op_index,op_index,op_reg_index_inc);
    instructions.emit_instruction(IR_OP_STORE,op_index_dest,op_index);
...
void IR_Translator_Statement::translate_statement_fork_to_ir_instructions(struct statement * stmt, bool from_subprogram){
    ...
    // -- 3. Generar operando de id de thread
    IR_operand op_1 = instructions.emit_operand_id_thread(id_thread_manager.get_last_id());
...
void IR_Translator_Statement::translate_statement_sleep_to_ir_instructions(struct statement * stmt, bool from_subprogram){
    IR_operand op_reg = instructions.emit_operand_register(expr_translator.translate(stmt->stmt.statement_sleep.sleep_expr));

    instructions.emit_instruction(IR_SLEEP_PROCESS,op_reg);
}
