 ...
 * @brief Implementacion de bloque de memoria para la maquina virtual de lamport
 */

#include "LVM/memory_block.hpp"

// ===============================================================

// ----- IMPLEMENTACION DE METODOS PRIVADOS [MEMORY BLOCK] -----

// ===============================================================

// ----- IMPLEMENTACION DE METODOS PUBLICOS [MEMORY BLOCK] -----

std::string LVM_Memory_Block::get_type_of_alloc_str() const{
    std::string allocates_type_str;

    auto it = LVM_Memory_Block_type_t_str.find(this->allocates_type);
    if(it != LVM_Memory_Block_type_t_str.end())
        allocates_type_str = it->second;

    return allocates_type_str;
}

std::string LVM_Memory_Block::get_value_str() const{
    std::string value_allocated_str;
    switch (this->allocates_type)
    {
    case MEM_BLOCK_EMPTY:
    {
