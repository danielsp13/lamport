...
#include "LVM/memory/initializer.hpp"
...
void LVM_Initializer::create_threads(){
    ...
    for(int i=1; i<instructions.size(); i++){
        ...
        if(instructions[i].get_code_instr() == IR_START_PROCESS || instructions[i].get_code_instr() == IR_LABEL){
            pc_mth = i-1;
            break;
        }
    }

    thread_manager.create_master_thread(pc_mth);
    

    for(int i=pc_mth-1; i<instructions.size(); i++){
        ...
        // -- Obtener atributos de instruccion
        int id = instr.get_operand_1().value().get_address();
        ...
        for(int j=i+1; j<instructions.size(); j++){
            ...
            // -- Comprobar si ha finalizado el proceso
            if(frwd_instr.get_code_instr() == IR_END_PROCESS){
                if(frwd_instr.get_operand_1().value().get_address() == id)
                    break;
            }
        }

        // -- Crear nueva hebra
        thread_manager.create_thread(id,i+1,segment_addr++,nullptr,1,0,child_counter);
    }
}